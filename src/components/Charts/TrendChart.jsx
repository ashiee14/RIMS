...
const safeData = safeData.map((d) => ({
  year: d.year,
  indexedPubs: Number(d.indexedPubs) || 0,
  crossrefCitations: Number(d.crossrefCitations) || 0,
  scopusCitations: Number(d.scopusCitations) || 0,
}));

export function TrendChart({ safeData = [] }) {
  if (!safeData.length) return null;

  const W = 500, H = 180;
  const pad = { t: 10, b: 30, l: 40, r: 10 };

  const allVals = safeData.flatMap((d) => [d.indexedPubs, d.crossrefCitations, d.scopusCitations]);
  const maxVal  = Math.max(...allVals, 1);
  const maxY    = Math.ceil(maxVal / 100) * 100 || 100;

  const xPos = (i) =>  safeData.length === 1 ? W / 2 : pad.l + (i / (safeData.length - 1)) * (W - pad.l - pad.r);
  const yPos = (v) => pad.t + (1 - v / maxY) * (H - pad.t - pad.b);
  const bw   = (W - pad.l - pad.r) / (safeData.length + 1);

  const pts1 = safeData.map((d, i) => `${xPos(i)},${yPos(d.crossrefCitations)}`).join(" ");
  const pts2 = safeData.map((d, i) => `${xPos(i)},${yPos(d.scopusCitations)}`).join(" ");
